...
class TimeoutSubmissionTests(TestCase):
    ...
    def test_expired_practice_page_redirects_to_result(self):
        attempt = build_submitted_attempt(
            self.student, self.topic, self.admin,
            15, 5, 0,
        )

        old_start = timezone.now() - timezone.timedelta(minutes=132)
        ...
        response = self.client.get(
            reverse('practice_attempt', args=[attempt.id])
        )
        ...
        self.assertRedirects(
            response,
            reverse('practice_result', args=[attempt.id]),
            fetch_redirect_response=False,
        )

        self.assertEqual(attempt.status, 'submitted')
        self.assertEqual(attempt.score, 15)
        self.assertEqual(attempt.end_time, old_start + timezone.timedelta(minutes=30))
        ...
        self.assertEqual(result.context['time_taken'], '30 min 00 sec')

        answer = attempt.answers.first()

        late_save = self.client.post(
            reverse('practice_answer_save', args=[attempt.id]),
            data=json.dumps({
        ...
        self.assertEqual(late_save.status_code, 409)

    def test_expired_timer_request_finalizes_attempt(self):
        attempt = build_submitted_attempt(
            self.student, self.topic, self.admin,
            0, 0, 20,
        )
        ...
        response = self.client.get(
            reverse('practice_timer_status', args=[attempt.id])
        )
        ...
        self.assertEqual(
            attempt.end_time,
            old_start + timezone.timedelta(minutes=30),
        )
        self.assertEqual(response.json()['remaining_seconds'], 0)
        self.assertEqual(response.json()['attempt_status'], 'submitted')
